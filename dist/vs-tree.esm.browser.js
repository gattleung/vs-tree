...
var parseTemplate = function parseTemplate(name, ctx) {
  var slotName = ctx.store.slots[name];

  if (slotName) {
    var node = slotName.node.cloneNode(true);
    node.classList.add('vs-tree-text');
    node.setAttribute('tree-node-id', ctx.id);
    ctx.__buffer = {};
    var prefix = "\n        var ".concat(slotName.scope, " = _;\n      ");
    slotName.text.replace(slotName.interpolate, function (a, b) {
      prefix += "_.__buffer['".concat(a, "'] = ").concat(b, ";");
    }); // eslint-disable-next-line no-new-func

    var render = new Function('_', prefix);
    render.call(ctx, ctx);
    node.innerText = node.innerText.replace(slotName.interpolate, function (a) {
...
var Node = /*#__PURE__*/function () {
  ...
  _createClass(Node, [{
    ...
    value: function createNode() {
      ...
      var slotAppend = parseTemplate('append', this);
      ...
      dom.addEventListener('click', function (e) {
        ...
        if (_this.store.checkOnClickNode && !_this.disabled && !(_this.store.breadcrumb && !_this.isLeaf)) {
        ...
        if (_this.store.breadcrumb && !_this.isLeaf) {
          _this.store.breadcrumb.list.push(_this);
    ...
    value: function createText() {
      var slot = parseTemplate('name', this);
    ...
    key: "updateChecked",
    value: function updateChecked(check, isInitDefault) {
      if (!isInitDefault && this.disabled) return;
      if (!this.store.showCheckbox) return; // if (this.disabled) return

      this.checked = check;
      this.sortId = Date.now();
      this.checkboxNode && (this.checkboxNode.checked = check);
      this.dom && this.dom.classList.remove('is-indeterminate'); // 验证关联关系
    ...
    value: function updateCheckedParent(_checked, isInitDefault) {
      if (!isInitDefault && this.disabled) return;
    ...
    value: function updateRadioChecked(checked, isInitDefault) {
      if (!isInitDefault && this.disabled) return;
    ...
    value: function loadData(callback) {
      ...
      var resolve = function resolve() {
        ...
        } else if (_this9.loadingEl) {
          _this9.loadingEl.classList.remove('is-loading');
        }
...
var TreeStore = /*#__PURE__*/function () {
  ...
  _createClass(TreeStore, [{
    ...
    key: "allowEmit",
    value: function allowEmit(check, type) {
      var _this$checkboxType = this.checkboxType,
          Y = _this$checkboxType.Y,
          N = _this$checkboxType.N;
...
var BreadcrumbItem = /*#__PURE__*/function () {
  function BreadcrumbItem(node, parent) {
    ...
    var _this$parent$options = this.parent.options,
        icon = _this$parent$options.icon,
        link = _this$parent$options.link,
        _this$parent$options$ = _this$parent$options.separator,
        separator = _this$parent$options$ === void 0 ? '/' : _this$parent$options$;
  ...
  _createClass(Breadcrumb, [{
    ...
    value: function renderBreadcrumb() {
      ...
      var _this$options = this.options,
          el = _this$options.el,
          _this$options$change = _this$options.change,
          change = _this$options$change === void 0 ? function () {} : _this$options$change;
      ...
      var bs = this.list.map(function (node) {
        return new BreadcrumbItem(node, _this).createDom();
      });
      _el.innerHTML = '';
...
var Tree = /*#__PURE__*/function () {
  function Tree(selector, ops) {
    ...
    var slotsMap = {};
    var slots = this.$el.querySelectorAll('[tree-slot]');

    if (slots && slots.length) {
      slots.forEach(function (v) {
        var name = v.attributes['tree-slot'].value;
        var scope = v.attributes['tree-slot-scope'].value;
        slotsMap[name] = {
          scope: scope,
          node: v,
          interpolate: _this.interpolate,
          text: v.innerText,
          inner: v.outerHTML
        };
    ...
    var _ref = ops.virtual || {},
        _ref$showCount = _ref.showCount,
        showCount = _ref$showCount === void 0 ? 20 : _ref$showCount,
        _ref$itemHeight = _ref.itemHeight,
        itemHeight = _ref$itemHeight === void 0 ? 26 : _ref$itemHeight,
        _ref$maxHeight = _ref.maxHeight,
        maxHeight = _ref$maxHeight === void 0 ? '400px' : _ref$maxHeight,
        _ref$minHeight = _ref.minHeight,
        minHeight = _ref$minHeight === void 0 ? '0px' : _ref$minHeight; // 每一项的高度


    this.itemHeight = itemHeight; // 当前可见数量

    this.showCount = showCount; // 最大高度

    this.maxHeight = ops.maxHeight || maxHeight; // 最小高度

    this.minHeight = ops.minHeight || minHeight; // 当前可见列表
    ...
    var start = function start() {
      _this.store = new TreeStore({
        ...
        max: ops.max,
        slots: slotsMap,
        breadcrumb: _this.$$breadcrumb || null,
  ...
  _createClass(Tree, [{
    ...
    value: function _render() {
      ...
      if (this.$$breadcrumb) {
        var current = this.$$breadcrumb.current;
        this.data = this.nodes.filter(function (v) {
          return v.parent && v.parent.id === current.id;
        });
        this.$$breadcrumb.renderBreadcrumb();
      } else {
        this.data = this.nodes.filter(function (v) {
          // 过滤隐藏节点 ｜ 隐藏root节点
          return _this2._hasKeyword(v) && v.visbile && !(_this2.store.hideRoot && v.level === 0);
        });
      }
    ...
    key: "_hasKeyword",
    value: function _hasKeyword(v) {
      var _this3 = this;

      if (!this.keyword) return true;

      var boo = this._checkFilter(v);

      if (!boo) {
        v.childNodes.forEach(function (node) {
          if (!boo) {
            boo = _this3._hasKeyword(node);
          }
        });
      } else {
    ...
    value: function filter() {
      ...
      if (this.store.onlySearchLeaf) {
        var data = this.nodes.filter(function (v) {
          return !v.childNodes.length && _this4._checkFilter(v) && !(_this4.store.hideRoot && v.level === 0);
        });
